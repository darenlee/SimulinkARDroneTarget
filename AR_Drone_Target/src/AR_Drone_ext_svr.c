...
/*************************************
 * Modified EXT Mode Parameters for AR Drone
**************************************/
#define AR_DRONE_GET_PKT_DELAY 50000L //used to add time delay before updating tunable parameter. 
                                       //This is used in function: PRIVATE const char *GetPkt(const int pktSize)
                                     ...
                                     const int pktSize)
{
	...
	printf("Inside ProcessSetParamPkt \n");  //DAREN :added this here
    pkt = GetPkt(pktSize);
    if (pkt == NULL) {
