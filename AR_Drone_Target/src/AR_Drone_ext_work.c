...
/*************************************
 * Start of Modified EXT Mode Parameters for AR Drone
**************************************/
//http://publib.boulder.ibm.com/infocenter/aix/v6r1/index.jsp?topic=%2Fcom.ibm.aix.prftungd%2Fdoc%2Fprftungd%2Fproc_thread_prior.htm
...
// extern ExtY_arm_cortex_a_external_mo_T arm_cortex_a_external_mode_Y;

#define AR_DRONE_DLY_INF_LOOP 375000L //used to add time delay in infinite loop while we wait for host 
                                       //This is used in function: rtExtModeWaitForStartPkt( )

/*************************************
 * End of Modified EXT Mode Parameters for AR Drone
**************************************/
                              ...
                              boolean_T      *stopReqPtr)
{
	...
	static int increment_me=0; //DAREN: Added this here
	int sem_value;  //DAREN: Added this here
	/*
    ...
    if (ExtWaitForStartPkt()) {
        while(!startModel && !(*stopReqPtr)) {
         ...
         // DAREN: Added this for debugging purposes of examining semaphore values
         // 
         // printf("The sem value is: %d \n",sem_value);
		 //	printf("i have incremented this many times waiting: %d , Current Time: %f \n",increment_me++); 
			
			usleep(AR_DRONE_DLY_INF_LOOP); //DAREN: added this here
            rt_ExtModeSleep(0L, 375000L); //DAREN: this does nothing
            rt_PktServerWork(ei,numSampTimes,stopReqPtr);
            ...
            sem_init(&baserateTaskSem,0,0); //DAREN: reset semaphore from updating, this is done to prevent linux scheduler 
                                            //from excessively unlocking base-rate task once we're done waiting for host
        }
    }
